# Test keyword extraction with text rank
...
#POS_KEEPS = POS_KEEPS_TWIT
#POS_LEMMA = POS_LEMMA_TWIT

UNIQ_WORDS = { ".": 0 }


def is_not_word (word):
    return PAT_PUNCT.match(word) or PAT_SPACE.match(word) or PAT_NUMBER.match(word)
...
def parse_plain_doc (doc_id, json_iter):
    """
    parse one plain text document to prep for TextRank
    """
    global DEBUG
    ...
    for meta in json_iter:
        ...
        for graf_text in filter_quotes(meta, is_email=False):
            if DEBUG:
                print("graf_text:", graf_text)

            grafs, new_base_idx = parse_graf(doc_id, graf_text, base_idx)
            base_idx = new_base_idx
...
def stage1(process_folder, text, label):
    
    path_stage1 =process_folder + label + 'stage1.json'

    with open(path_stage1, 'w') as f:
        for graf in parse_plain_doc('mck_text',iter(text.splitlines())):
...
# Stage 4
def stage4phrases(process_folder, label):
    path_stage2 = process_folder + label + 'stage2.json'

    path_stage4 = process_folder + label + 'keywords.txt'

    phrases = ", ".join(set([p for p in limit_keyphrases(path_stage2, phrase_limit=50)]))
    with open(path_stage4, 'w') as f:
        f.write(phrases)
...
# Stage 4
def stage4excerpt(process_folder, label):
    path_stage3 = process_folder + label + 'stage3.json'
